...
import { ArrowLeft, Calculator, DollarSign, HandHeart, TrendingUp, Info, CheckCircle } from 'lucide-react';
...
const Zakat: React.FC = () => {
  ...
  const zakatRecipients = [
    { title: 'The Poor (Al-Fuqara)', description: 'Those who have little to no income' },
    { title: 'The Needy (Al-Masakin)', description: 'Those who cannot meet basic needs' },
    { title: 'Zakat Administrators', description: 'Those who collect and distribute Zakat' },
    { title: 'New Muslims', description: 'Those whose hearts are to be reconciled' },
    ...
    { title: 'In the Way of Allah', description: 'For causes that serve Islam' },
      ...
      <div className="p-4 space-y-6">
        ...
        <header className="flex items-center gap-4 py-2">
          ...
          <div>
            ...
            <p className="text-sm text-primary-foreground/70">Purify your wealth</p>
          </div>
        </header>
        ...
        <div className="glass rounded-3xl p-5 border border-primary-foreground/10 shadow-card">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="text-lg font-bold text-gradient-gold">What is Zakat?</h2>
        ...
        <div className="glass rounded-3xl p-5 border border-primary-foreground/10 shadow-card">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-lg font-bold text-gradient-gold">Zakat Calculator</h3>
          </div>

          <div className="space-y-3">
            <div>
              <label className="text-xs text-primary-foreground/70 mb-1 block">Cash & Savings ($)</label>
              <Input
                type="number"
                placeholder="0.00"
                value={savings}
                onChange={(e) => setSavings(e.target.value)}
                className="glass border-primary-foreground/20 text-primary-foreground"
              />
            </div>
            <div>
              <label className="text-xs text-primary-foreground/70 mb-1 block">Gold Value ($)</label>
              <Input
                type="number"
                placeholder="0.00"
                value={gold}
            ...
            <div>
              <label className="text-xs text-primary-foreground/70 mb-1 block">Silver Value ($)</label>
              <Input
                type="number"
                placeholder="0.00"
                value={silver}
            ...
            <div>
              <label className="text-xs text-primary-foreground/70 mb-1 block">Investments & Stocks ($)</label>
              <Input
                type="number"
                placeholder="0.00"
                value={investments}
            ...
            <div>
              <label className="text-xs text-primary-foreground/70 mb-1 block">Business Assets ($)</label>
              <Input
                type="number"
                placeholder="0.00"
            ...
            <div>
              <label className="text-xs text-primary-foreground/70 mb-1 block">Debts to Deduct ($)</label>
              <Input
                type="number"
                placeholder="0.00"
                value={debts}
            ...
            {zakatAmount !== null && (
              <div className="mt-4 p-4 glass rounded-2xl border border-islamic-gold/30 bg-islamic-gold/10">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm text-primary-foreground/70">Your Zakat Due:</span>
                </div>
                <p className="text-3xl font-bold text-gradient-gold">${zakatAmount.toFixed(2)}</p>
                {zakatAmount === 0 && (
        ...
        {/* Recipients */}
        <div className="glass rounded-3xl p-5 border border-primary-foreground/10 shadow-card">
          <h3 className="text-lg font-bold text-gradient-gold mb-4">8 Categories of Zakat Recipients</h3>
          <div className="space-y-3">
        ...
        <div className="glass rounded-3xl p-5 border border-primary-foreground/10 shadow-card">
          ...
          <ul className="space-y-2 text-sm text-primary-foreground/80">
            ...
            <li>• Consult with local scholars for complex financial situations</li>
          </ul>
        </div>
      </div>
    </MobileLayout>
