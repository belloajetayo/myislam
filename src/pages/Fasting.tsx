...
const Fasting: React.FC = () => {
  ...
  const toggleFasting = async () => {
    ...
    setFastingDays(prev => todayFasted ? prev - 1 : prev + 1);
    toast.success(todayFasted ? 'Fasting unmarked' : 'MashaAllah! Fast recorded');
  };

  const fastingGuide = [
    { title: 'Make Intention (Niyyah)', description: 'Make sincere intention before Fajr to fast for the sake of Allah', icon: '🌙' },
    { title: 'Eat Suhoor', description: 'Have a pre-dawn meal before Fajr time. The Prophet ﷺ said there is blessing in Suhoor', icon: '🍽️' },
    { title: 'Stop at Fajr', description: 'Stop eating and drinking when Fajr time begins', icon: '🌅' },
    { title: 'Continue Your Day', description: 'Abstain from food, drink, and other invalidators until Maghrib', icon: '☀️' },
    { title: 'Break Fast at Maghrib', description: 'Break your fast when Maghrib time enters, preferably with dates and water', icon: '🌆' },
    { title: 'Make Dua', description: "Say: 'Dhahaba adh-dhama' wa abtallatil-'urooq wa thabatal-ajru in sha Allah'", icon: '🤲' },
  ];
  ...
  return (
    <MobileLayout showNav={false}>
      <div className="p-4 space-y-6">
        ...
        <div className="glass rounded-3xl p-5 border border-primary-foreground/10 shadow-card">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-gradient-gold">Fasting Tracker</h2>
                <p className="text-xs text-primary-foreground/70">Track your fasting days</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-2xl font-bold text-gradient-gold">{fastingDays}</p>
        ...
        {/* How to Fast Guide */}
        <div className="glass rounded-3xl p-5 border border-primary-foreground/10 shadow-card">
          <h3 className="text-lg font-bold text-gradient-gold mb-4">How to Fast</h3>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="glass rounded-2xl p-4 border border-primary-foreground/10">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="text-sm font-semibold text-gradient-gold">Rewarded Actions</h4>
            </div>
          ...
          <div className="glass rounded-2xl p-4 border border-primary-foreground/10">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="text-sm font-semibold text-gradient-gold">Things to Avoid</h4>
            </div>
            <ul className="space-y-2">
