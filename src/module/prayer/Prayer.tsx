...
const Prayer: React.FC = () => {
  ...
  const playTestAdhan = async () => {
    ...
    if (!played) {
      toast.warning("Notification sent, but your browser blocked adhan audio. Tap the notification to try again.");
    }
    return played;
  };

  const showLocalTestNotification = async () => {
    const notification = new Notification("🕌 Test Prayer Notification", {
    ...
    const played = await playTestAdhan();
    toast.success(played ? "Local test notification sent and adhan started." : "Local test notification sent.");
  };
  ...
  const handleSendTestNotification = async () => {
    try {
      ...
      if (!fnError && result?.sent > 0) {
        const played = await playTestAdhan();
        toast.success(played ? "Test notification sent and adhan started." : "Test notification sent! Tap it to try adhan playback.");
      } else {
