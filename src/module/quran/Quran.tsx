...
const Quran: React.FC = () => {
  ...
  useEffect(() => {
    if (!Number.isInteger(routeSurahNumber) || routeSurahNumber < 1 || routeSurahNumber > 114) {
      return;
    }
    ...
    fetchSurahDetailRef.current(routeSurahNumber, selectedReciter)
      .then((detail) => {
        if (!detail || cancelled) return;
        setSelectedSurah(detail);
        localStorage.setItem("myislam_last_read_surah_number", routeSurahNumber.toString());
        localStorage.setItem("myislam_last_read_surah_name", detail.englishName);
        localStorage.setItem("myislam_last_read_surah_translation", detail.englishNameTranslation);
        setLastReadSurah({
          number: routeSurahNumber,
          name: detail.englishName,
          translation: detail.englishNameTranslation,
        });
      })
  ...
  }, [routeSurahNumber, selectedReciter]);
  ...
  useEffect(() => {
    if (!selectedSurah || !Number.isInteger(routeAyahNumber)) return;

    const targetAyahNumber = Math.min(
      Math.max(routeAyahNumber, 1),
      selectedSurah.ayahs.length,
    );

    window.requestAnimationFrame(() => {
      document
        .getElementById(`ayah-${selectedSurah.number}-${targetAyahNumber}`)
        ?.scrollIntoView({ behavior: "smooth", block: "center" });
    });
  }, [selectedSurah, routeAyahNumber, arabicOnlyMode]);

  const scrollToFirstAyah = () => {
    if (!selectedSurah) return;
    document
      .getElementById(`ayah-${selectedSurah.number}-1`)
      ?.scrollIntoView({ behavior: "smooth", block: "center" });
    setSearchParams({ surah: selectedSurah.number.toString(), ayah: "1" }, { replace: true });
  };
    ...
    return (
      <MobileLayout showNav={false}>
        ...
        {!verseMode && (
          ...
          <button
            onClick={scrollToFirstAyah}
            aria-label="Go to first ayah"
            title="Go to first ayah"
            className="fixed right-5 bottom-5 z-30 w-11 h-11 rounded-full gradient-primary shadow-lg flex items-center justify-center text-primary-foreground border border-white/20 hover:scale-105 active:scale-95 transition-transform"
          >
